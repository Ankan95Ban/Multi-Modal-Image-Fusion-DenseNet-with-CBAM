...
def _ssim(img1, img2, img3, window, window_size, channel, size_average=True):
    ...
    Calculates SSIM.
    img1: Fused Image
    img2: Source Image A (e.g., IR)
    img3: Source Image B (e.g., VIS)
    """
    ...
    # SSIM map calculation for both sources
    ssim_map_12 = (2 * sigma12 + C2) / (sigma1_sq + sigma2_sq + C2)
    ssim_map_13 = (2 * sigma13 + C2) / (sigma1_sq + sigma3_sq + C2)

    # Selection logic: maximize structural similarity relative to the "stronger" local source area
    if size_average:
